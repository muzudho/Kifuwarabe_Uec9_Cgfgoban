...
__declspec(dllimport) int cgfgui_thinking(
	int init_board[],	// 初期盤面
...
//			SendMessage(hWnd,WM_COMMAND,IDM_THINKING,0L); DestroyWindow(hWnd);	// Profile用のテスト
			...
			if ( ret != 0 ) {
				...
				if ( ret == 1 ) sprintf(sTmp,"(^q^)DLL is not found.");
				else            sprintf(sTmp,"(^q^)Fail GetProcAddress() ... %d",ret);
...
#ifdef CHECK_LOG
	// muzudho: ログ出力のために。
	ofstream outputfile("muzudho_cgfgoban_log.txt");
	outputfile << "called: ThinkingDLL";
#endif
